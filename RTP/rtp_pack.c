...
struct pac_handle
{
    ...
    unsigned int ts_start_millisec;		// timestamp in millisecond
    unsigned int ts_current_sample;		// timestamp in 1/90000.0 unit

    struct pac_param params;
};

static unsigned int get_current_millisec(void)
{
	struct timeb tb;
	ftime(&tb);
...
//		dump_nalu(&handle->nalu);
        ...
        handle->ts_current_sample = (unsigned int) ((get_current_millisec() - handle->ts_start_millisec) * 90.0);    // calculate the timestamp for a new NALU
