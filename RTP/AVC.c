...
static const unsigned char *ff_avc_find_startcode_internal(const unsigned char *p, const unsigned char *end)
{
    const unsigned char *a = p + 4 - ((intptr_t)p & 3);  // a=p后面第一个地址为00的位置上
    ...
    for (end -= 3; p < end; p += 4) {
        unsigned long x = *(const unsigned long*)p;  // 取4个字节
