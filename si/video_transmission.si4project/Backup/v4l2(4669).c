...
static int g_aiSupportedFormats[] = {V4L2_PIX_FMT_YUV420, V4L2_PIX_FMT_MJPEG, V4L2_PIX_FMT_RGB565};
...
static int V4l2InitDevice(char *strDevName, PT_VideoDevice ptVideoDevice)
{
    ...
    tV4l2Fmt.fmt.pix.width       = iwidth;
    tV4l2Fmt.fmt.pix.height      = iheigt;
	...
	printf("ptVideoDevice->iWidth = %d\n",ptVideoDevice->iWidth);
	printf("ptVideoDevice->iHeight = %d\n",ptVideoDevice->iHeight);
    /* request buffers */
...
static int V4l2GetFrameForStreaming(PT_VideoDevice ptVideoDevice, PT_VideoBuf ptVideoBuf)
{
    ...
    ptVideoBuf->tPixelDatas.iBpp    = (ptVideoDevice->iPixelFormat == V4L2_PIX_FMT_YUV420) ? 16 : \
...
int V4l2Init(void)
{
    return register_video_opr(&g_tV4l2VideoOpr);
}
